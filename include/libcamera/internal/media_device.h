...
public:
    ...
    MediaEntity *getEntityById(const int id) const;
    MediaEntity *getEntityByName(const std::string &name) const;
    MediaEntity *getEntityByVideoNode(const int &videoNode) const;
    MediaLink *link(const std::string &sourceName, unsigned int sourceIdx,
                    const std::string &sinkName, unsigned int sinkIdx);
    MediaLink *link(const MediaEntity *source, unsigned int sourceIdx,
                    const MediaEntity *sink, unsigned int sinkIdx);
    MediaLink *link(const MediaPad *source, const MediaPad *sink);
    int getSensorEntityIdxByEntity(MediaEntity *mediaEntity);
    int getSensorEntityIdxByEntityHelper(MediaEntity *mediaEntity);
